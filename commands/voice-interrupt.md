---
description: Interrupt Voice Code and add spoken guidance
---

Interrupt the current VoiceBridge audio and collect one fresh spoken
instruction with `mcp__voicebridge__voice_interrupt`. This command is intended
...
2. If it returns `ok: false`, show the error. If Voice Code is inactive, tell
   the user to run `/voicebridge:voice-code`; do not start it implicitly.
...
5. On `end_reason: "device_error"`, show the microphone problem and leave Voice
   Code active so the user can retry or call `voice_stop`.
6. If the transcript clearly asks to end Voice Code entirely, call
   `mcp__voicebridge__voice_stop` and do not resume the task.

After accepting the guidance, continue the normal Voice Code workflow: work
