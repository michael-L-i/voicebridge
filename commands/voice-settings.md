---
description: Choose Cadence Code speech models
---

Open Cadence Code Settings. This command changes local TTS and STT model
...
2. If `ready` is true, explain that changing models ends the active Cadence Code
   session. Ask for confirmation using Claude Code's structured question UI. If
   the user confirms, call `mcp__cadence-code__voice_stop`; otherwise end without
   changing settings.
...
4. If the user cancels either selector, end without changing settings.
   Otherwise call `mcp__cadence-code__voice_configure` with the chosen TTS and
   STT IDs.
5. Report the selected labels and say the models download when Start Talking next
   runs. Do not call `mcp__cadence-code__voice_start`, `voice_speak`, or
