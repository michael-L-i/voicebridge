...
3. Call `mcp__voicebridge__voice_models`. Present separate TTS and STT
   single-choice selectors using Claude Code's structured question UI. Preserve
   the returned lightest-to-heaviest order. Each choice must show its tier,
   download size, and short description. Preselect the IDs in `current`; when
   an existing custom model is not in the catalog, preselect `defaults`.
