...
the Cadence Code boundary; Codex and Claude Code remain separate products with
...
After local transcription, Cadence Code returns the text to Codex or Claude Code
...
logs, conversation history, and telemetry are outside Cadence Code's boundary.

## Local files and downloads

Cadence Code stores configuration and its private Python environment locally.
Codex uses `~/.cadence-code`; Claude Code supplies a persistent per-plugin data
...
dependencies, and model weights, not Cadence Code recordings or transcripts.
